...
def run(meta, config, starting_noise=None):
    ...
    for image_id, sample in zip(image_ids, samples_fake):
        ...
        output_images.append(sample)
        # sample.save(  os.path.join(output_folder, img_name)   )

    return output_images

def run_inpainting(inputs):
    meta_list = [ 
        # - - - - - - - - GLIGEN on image grounding for inpainting - - - - - - - - # 
        dict(
            ckpt = "../pretrained_models/image_inpainting.bin",
            input_image = inputs.base_image_path,
            prompt = "",
            images = inputs.inpainting_content,
            locations = inputs.inpainting_masks, # mask will be derived from box 
            save_folder_name=""
        ),
    ]
    ...
    meta = meta_list[0]
    result = run(meta, args, starting_noise)
